...
function _registerHealthRoute(app) {
  app.get("/health", (_req, res) => {
    ...
    if (_ghCacheClient) {
      health.githubCache = {
        installed: true,
        version: _ghCacheClient.CACHE_SDK_VERSION,
        rateLimit: _ghCacheClient.getRateLimitStatus(),
        cache: _ghCacheClient.getCacheStats(),
      };
    }
    res.json(health);
  });
...
function init(app, options) {
  ...
  if (app) {
    ...
    try {
      var fs = require("fs");
      var cacheClientPath = require("path").resolve(process.cwd(), "github-cache-client.js");
      if (fs.existsSync(cacheClientPath)) {
        var _mod = require(cacheClientPath);
        if (_mod && typeof _mod.getRateLimitStatus === "function" && typeof _mod.getCacheStats === "function") {
          _ghCacheClient = _mod;
          if (typeof _mod.registerRoutes === "function") {
            _mod.registerRoutes(app);
          }
          console.log("[hub-sdk] GitHub Cache Client v" + (_ghCacheClient.CACHE_SDK_VERSION || "unknown") + " loaded");
        } else {
          console.warn("[hub-sdk] GitHub Cache Client found but missing expected exports (getRateLimitStatus, getCacheStats) — skipping");
        }
      }
    } catch (err) {
      console.warn("[hub-sdk] GitHub Cache Client not loaded:", err.message);
    }
...
module.exports = {
  ...
  // GitHub Cache Client (auto-loaded if github-cache-client.js present)
  getGitHubCacheClient: function() { return _ghCacheClient; },
