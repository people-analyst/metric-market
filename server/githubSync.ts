...
// ── Pull from GitHub (additive only — never deletes local files) ────

let _lastPullTime: number | null = null;
let _lastPullSha: string | null = null;
let _pullCount = 0;

const PULL_SKIP_DIRS = new Set(["node_modules", ".git", ".cache", ".config", ".local", "dist"]);
const PULL_SKIP_FILES = new Set([".replit", "replit.nix", ".replit.nix", "replit.md"]);

async function pullFromGitHub(): Promise<{
  success: boolean;
  sha?: string;
  filesUpdated?: number;
  error?: string;
  ...
  try {
    ...
    try {
      ...
      return { success: false, error: "Could not find main branch on GitHub" };
    }

    if (headSha === _lastPullSha) {
      return { success: true, sha: headSha, filesUpdated: 0 };
    }

    console.log(`[github-sync] Pulling from ${REPO_OWNER}/${REPO_NAME} (${headSha.substring(0, 7)})...`);

    const { data: commitData } = await octokit.rest.git.getCommit({
      owner: REPO_OWNER, repo: REPO_NAME, commit_sha: headSha,
    });
    const { data: tree } = await octokit.rest.git.getTree({
      owner: REPO_OWNER, repo: REPO_NAME, tree_sha: commitData.tree.sha, recursive: "1",
    });
    ...
    for (const item of tree.tree) {
      ...
      if (PULL_SKIP_DIRS.has(topDir) || PULL_SKIP_FILES.has(item.path)) continue;
      ...
      if (fs.existsSync(fullPath)) {
        ...
        const header = `blob ${localContent.length}\0`;
        const localGitSha = createHash("sha1")
          .update(Buffer.concat([Buffer.from(header), localContent]))
          .digest("hex");
        if (localGitSha === item.sha) needsUpdate = false;
      }

      if (needsUpdate) {
        const { data: blob } = await octokit.rest.git.getBlob({
          owner: REPO_OWNER, repo: REPO_NAME, file_sha: item.sha,
        });
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        if (blob.encoding === "base64") {
          fs.writeFileSync(fullPath, Buffer.from(blob.content, "base64"));
        } else {
    ...
    _lastPullSha = headSha;
    _pullCount++;

    console.log(`[github-sync] Pull complete: ${filesUpdated} files updated (${headSha.substring(0, 7)})`);
    return { success: true, sha: headSha, filesUpdated };
  } catch (err: any) {
    const errorMsg = err.message || String(err);
