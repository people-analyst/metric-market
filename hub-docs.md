...
- **Component Export System:** Discoverable component registry and export packaging for cross-app integration. API endpoints: `GET /api/components` (registry), `GET /api/components/:key` (full detail + schemas + integration guide), `GET /api/export/:key` (downloadable package). Includes formal data contracts for Conductor (input producer), AnyComp (output consumer), and Metric Engine (bidirectional metric definition standardization), with field mappings, example payloads, and integration guides. Frontend at `/export`.
- **Range Builder Control:** Interactive compensation range simulator with KPI Index cards (Cost Impact, Peer Equity, Competitiveness, People Impact). Supports Standard and Custom (2-10) level structures. Located at `/range-builder`. Integration: Conductor supplies market P50/P75 + employee data; AnyComp consumes adjusted ranges + KPI outputs for strategy modeling; Metric Engine provides canonical metric definitions and receives computed KPI values for benchmarking.
